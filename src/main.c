...
static bool validateProgram(string program_file, string main_f)
{
   ...
   gp_program = reverse(gp_program);
      ...
      bool semantic_error = analyseProgram(gp_program, true, program_file, main_f);
...
void printMakeFile(string output_dir, string install_dir, bool main_p)
{
   FILE *makefile = NULL;
   if(main_p){
     ...
     int length = strlen(output_dir) + 14;
     char makefile_name[length];
     strcpy(makefile_name, output_dir);
     strcat(makefile_name, "/");
     strcat(makefile_name, "Makefile.demo");
     makefile = fopen(makefile_name, "w");
   }
   if(makefile == NULL)
   {
...
int main(int argc, char **argv)
{
   string const usage = "Usage:\n"
                        "gp2 [-c] [-d] [-l <rootdir>] [-o <outdir>] [-m <mainfile>] <program_file>\n"
                        "gp2 -p <program_file>\n"
                        "gp2 -r <rule_file>\n"
                        "gp2 -h <host_file>\n\n"
                        "Flags:\n"
   ...
   bool validate = false, main_p = true;
   string program_file = NULL, host_file = NULL, rule_file = NULL,
          install_dir = NULL, output_dir = NULL, main_file = NULL;
   ...
   if(validate)
   {
      ...
      {
         bool result = validateProgram(program_file, main_file);
   ...
   {
      bool valid_program = validateProgram(program_file, main_file);
      if(!valid_program)
      {
      ...
      else
      {
         ...
         generateRules(gp_program, output_dir, main_file, main_p);

         print_to_console("Generating run time...\n");

         generateRuntimeMain(gp_program, output_dir, main_file, main_p, max_nodes, max_edges);
         print_to_console("Generating make file...\n");
         printMakeFile(output_dir, install_dir, main_p);
      }
   }
   if(yyin != NULL) fclose(yyin);
   if(gp_program) freeAST(gp_program);
