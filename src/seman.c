...
bool analyseProgram(List *gp_program, bool debug, string program_file, string main_f)
{
   ...
   * with the function freeSymbolList defined in the symbol module. */
   ...
   bool abort = declarationScan(gp_program, "Main", main_f);
   if(abort) return false;
   abort = semanticCheck(gp_program, "Main", main_f);
      ...
      switch(ast->declaration->type)
      {
         ...
         case RULE_DECLARATION:
         {
  	      string old_rule_name = ast->declaration->rule->name;
              string rule_name = makeRuleIdentifier(old_rule_name, scope, main_f);
...
{
   int length = strlen(rule_name) + strlen(main_f) + 2;
   ...
   new_rule_name = strcpy(new_rule_name, main_f);
   ...
   while(declarations)
   {
      ...
      switch(current_declaration->type)
      {
		 ...
		 commandScan(current_declaration->main_program,
                             scope, declarations, false, main_f);
...
void commandScan(GPCommand *command, string scope, List *declarations, bool in_loop, string main_f)
{
   switch(command->type)
   {
      case COMMAND_SEQUENCE:
      {
           ...
           while(command_list)
           {
              commandScan(command_list->command, scope, declarations, in_loop, main_f);
      ...
      case RULE_CALL:
      {
           ...
           GPRule *rule = findRuleDeclaration(declarations, name, procedure, main_f);
           if(rule == NULL)
           {
      ...
      case PROCEDURE_CALL:
      {
           ...
           else
           {
              ...
              commandScan(procedure->commands, name, declarations, in_loop, main_f);
           }
           break;
      }
      case IF_STATEMENT:
      case TRY_STATEMENT:
           commandScan(command->cond_branch.condition, scope, declarations, false, main_f);
           commandScan(command->cond_branch.then_command, scope, declarations, in_loop, main_f);
           commandScan(command->cond_branch.else_command, scope, declarations, in_loop, main_f);
           break;

      case ALAP_STATEMENT:
           commandScan(command->loop_stmt.loop_body, scope, declarations, true, main_f);
           break;

      case PROGRAM_OR:
           commandScan(command->or_stmt.left_command, scope, declarations, in_loop, main_f);
           commandScan(command->or_stmt.right_command, scope, declarations, in_loop, main_f);
           break;
                            ...
                            GPProcedure *procedure, string main_f)
{
   ...
   while(global_declarations != NULL)
   {
      ...
      if(declaration->type == RULE_DECLARATION)
      {
          ...
          if(strcmp(declaration->rule->name, name) == 0 ||
             strcmp(declaration->rule->name + strlen(main_f) + 1, name) == 0) return declaration->rule;
      }
