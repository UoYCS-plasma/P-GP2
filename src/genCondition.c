...
void generateConditionVariables(Condition *condition, string f_prefix, bool negate)
{
   ...
   switch(condition->type)
   {
      ...
      case 'n':
           PTF("/* NEG */\n");
           generateConditionVariables(condition->neg_condition, f_prefix, !negate);
           break;

      case 'a':
           PTF("/* AND */\n");
           generateConditionVariables(condition->left_condition, f_prefix, negate);
           generateConditionVariables(condition->right_condition, f_prefix, negate);
           break;
      case 'o':
           PTF("/* OR */\n");
           generateConditionVariables(condition->left_condition, f_prefix, negate);
...
void generateConditionEvaluator(Condition *condition, bool nested, string f_prefix, bool negate)
{
   ...
   {
      case 'e':
           PTF("%sb%d", f_prefix, bool_count++);
           break;

      case 'n':
           PTF("!");
           generateConditionEvaluator(condition->neg_condition, true, f_prefix, !negate);
           break;

      case 'a':
           if(nested) PTF("(");
           generateConditionEvaluator(condition->left_condition, true, f_prefix, negate);
           PTF(" && ");
           generateConditionEvaluator(condition->right_condition, true, f_prefix, negate);
           if(nested) PTF(")");
           break;
...
static void generatePredicateCode(Rule *rule, Predicate *predicate, string f_prefix)
{
   PTF("static void evaluatePredicate%s%d(Morphism *morphism)\n", f_prefix, predicate->bool_id);
   PTF("{\n");
   int index;
   ...
   {
      case INT_CHECK:
           PTFI("if(assignment_%d.type == 'i') %sb%d = true;\n", 3,
                predicate->variable_id, f_prefix, predicate->bool_id);
           PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           break;

      case CHAR_CHECK:
           PTFI("if(assignment_%d.type == 's' &&\n", 3, predicate->variable_id);
           PTFI("strlen(assignment_%d.string) == 1)\n", 6, predicate->variable_id);
           PTFI("%sb%d = true;\n", 6, f_prefix, predicate->bool_id);
           PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           break;

      case STRING_CHECK:
           PTFI("if(assignment_%d.type == 's') %sb%d = true;\n",
                3, predicate->variable_id, f_prefix, predicate->bool_id);
           PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           break;

      case ATOM_CHECK:
           PTFI("if(assignment_%d.type != 'l') %sb%d = true;\n",
                3, predicate->variable_id, f_prefix, predicate->bool_id);
           PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           break;
      ...
      case NOT_EQUAL:
      {
           ...
           {
              PTFI("if(", 3);
              generateIntExpression(left_label.list->first->atom, 1, false, f_prefix);
              if(predicate->type == EQUAL) PTF(" == ");
              if(predicate->type == NOT_EQUAL) PTF(" != ");
              generateIntExpression(right_label.list->first->atom, 1, false, f_prefix);
              PTF(") %sb%d = true;\n", f_prefix, predicate->bool_id);
              PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           }
           else
           {
              ...
              PTF("equalHostLists(array%d, array%d, list_length%d, list_length%d)) "
                  ...
                  list_count - 1, f_prefix, predicate->bool_id);
              PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           }
           break;
      }
      ...
      case LESS_EQUAL:
           PTFI("if(", 3);
           generateIntExpression(predicate->atom_comp.left_atom, 1, false, f_prefix);
           ...
           generateIntExpression(predicate->atom_comp.right_atom, 1, false, f_prefix);
           PTF(") %sb%d = true;\n", f_prefix, predicate->bool_id);
           PTFI("else %sb%d = false;\n", 3, f_prefix, predicate->bool_id);
           break;
...
void generatePredicateEvaluators(Rule *rule, Condition *condition, string f_prefix)
{
   ...
   {
      case 'e':
           generatePredicateCode(rule, condition->predicate, f_prefix);
           break;

      case 'n':
           generatePredicateEvaluators(rule, condition->neg_condition, f_prefix);
           break;

      case 'a':
      case 'o':
           generatePredicateEvaluators(rule, condition->left_condition, f_prefix);
           generatePredicateEvaluators(rule, condition->right_condition, f_prefix);
           break;
